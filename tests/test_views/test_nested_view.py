...
def test_delete_sub_view(jenkins_env):
    jk = Jenkins(jenkins_env["url"], (jenkins_env["admin_user"], jenkins_env["admin_token"]))
    parent = jk.create_view("test_create_sub_view", NestedView)
    with clean_view(parent):
        expected_view_name = "test_create_sub_view1"
...
def test_move_view(jenkins_env):
    jk = Jenkins(jenkins_env["url"], (jenkins_env["admin_user"], jenkins_env["admin_token"]))
    parent = jk.create_view("test_move_view", NestedView)
    with clean_view(parent):
        expected_view_name = "test_move_view_child1"
        original_child = jk.create_view(expected_view_name, ListView)
        ...
        with clean_view(new_child):
            logging.debug("Make sure deletion completes before continuing")
            async_assert(lambda: jk.find_view(expected_view_name) is None)

            # Make sure the new child has been created
            logging.debug("Performing validation")
            assert new_child.name == expected_view_name
            results = parent.views
            assert len(results) == 1
            assert results[0].name == expected_view_name

            logging.debug("Doing final validation...")
